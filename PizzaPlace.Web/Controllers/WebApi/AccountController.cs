...
namespace PizzaPlace.Web.Controllers.WebApi
{
    ...
    public class AccountController : ApiController
    {
        ...
        [Route("UserInfo")]
        public PizzaPlace.Web.Core.Dtos.UserInfoViewModel GetUserInfo()
        {
        ...
        // POST api/Account/ChangePasswordDto
            ...
            if (errors.Count == 0)
            {
                ...
                if (user == null)
                    return Request.CreateResponse(HttpStatusCode.NotFound,
                        new { error = "not_found", error_description = "Username not found." });

                string code = await UserManager.GeneratePasswordResetTokenAsync(user.Id);
                var callbackUrl = String.Format("{0}/#!/reset-password?userId={1}&code={2}", Request.RequestUri.GetLeftPart(UriPartial.Authority), user.Id, code);

                await UserManager.SendEmailAsync(
                    user.Id,
                    "Reset Password for Brocare Loyaltynow Admin",
        ...
        [AllowAnonymous]
        [Route("ResetPassword")]
        public async Task<HttpResponseMessage> ResetPassword(ResetPasswordBindingModel model)
        {
            List<string> errors = new List<string>();
            errors = ModelErrorChecker.Check(ModelState);
            ...
            if (errors.Count == 0)
            {
                ...
                if (user == null)
                    return Request.CreateResponse(HttpStatusCode.NotFound,
                        new { error = "not_found", error_description = "Your code is invalid. Kindly rechecked your inbox for new reset password request." });
                ...
                if (!result.Succeeded)
                {
                    foreach (var error in result.Errors)
                        return Request.CreateResponse(HttpStatusCode.NotAcceptable,
                           new { error = "failed", error_description = error + " Kindly try again." });
                }
        ...
        public string GetCurrentUserName()
        {
            ...
            return Request.GetOwinContext().Authentication.User.Identity.IsAuthenticated ? context.Administrators.SingleOrDefault(x => x.AdministratorId == userId).Name : "null";
        }
        ...
        //public IEnumerable<string> GetDisabled()
        //{
        ...
        private IHttpActionResult GetErrorResult(IdentityResult result)
        {
            ...
            if (!result.Succeeded)
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    //return BadRequest();
                    return Content(HttpStatusCode.NotAcceptable, new string[] { "Old Password does not match" });
                }
